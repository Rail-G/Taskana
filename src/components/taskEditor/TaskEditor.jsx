...
export default function TaskEditor() {
  const {setTasks, editType, create, createToggle, onClickChangeType} =
  ...
  const onSubmitEdit = (e) => {
    ...
    setTasks((prev) => {
      const updatedTasks = prev.map((item) => {
        return item.id == editType.task.id
          ? {
              id: editType.task.id,
              title: task,
              priority: Number(selectedBtn),
              createdAt: editType.task.createdAt,
              updatedAt: new Date().toISOString(),
            }
          : item;
      });
  ...
  return (
    <div className={`${styles.block} ${create && styles.active}`}>
      <form
        ...
        onSubmit={editType.type == "edit" ? onSubmitEdit : onSubmitCreate}
        noValidate
      >
        <div className={styles.headBlock}>
          <h1 className={styles.header}>
            {editType.type == "edit" ? "Редактировать" : "Создание задачи"}
          </h1>
          <TaskInput
            task={task}
            onChange={onChange}
        ...
        <div className={styles.submitBlock}>
          <button
            type="submit"
            className={`${styles.submitBtn} ${styles.createBtn}`}
            disabled={
              editType.task?.title == task.trim() &&
              editType.task?.priority == selectedBtn
                ? task
                : !task
            }
          >
            {editType.type == "edit" ? "Сохранить" : "Создать"}
          </button>
          <button
            type="button"
